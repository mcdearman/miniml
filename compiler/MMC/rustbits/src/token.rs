...
pub enum TokenKind {
    ...
    #[regex(r"-?0b[0-1]+", |lex| parse_radix_literal(lex, 2))]
    #[regex(r"-?0o[0-7]+", |lex| parse_radix_literal(lex, 8))]
    #[regex(r"-?0x[0-9a-fA-F]+", |lex| parse_radix_literal(lex, 16))]
    #[regex(r"-?([1-9]\d*|0)", |lex| lex.slice().parse().ok(), priority = 3)]
    Int(i64),
    #[regex(
        r"([0-9]*[.])?[0-9]+", 
...
fn parse_radix_literal(lex: &mut logos::Lexer<TokenKind>, radix: u32) -> Option<i64> {
    let slice = lex.slice();
    i64::from_str_radix(&slice[2..], radix).ok()
}
